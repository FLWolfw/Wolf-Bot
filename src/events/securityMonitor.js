...
async function processEvent(client, eventName, target) {
  const guild = target?.guild; if (!guild) return;
  const cfg = EVENT_CONFIG[eventName]; if (!cfg) return;
...
async function processAuditEntry(entry, guild) {
  ...
  const saved = await persistAuditLogEntry(guild.client.db, entry);
  if (!saved) logger.debug('Audit log entry could not be persisted', { guildId: guild.id, auditLogId: entry.id });
}
...
export function registerSecurityMonitor(client) {
  ...
  // Discord emits one audit-log event for administrative actions. This gives
  // Wolf an external copy of the full change set even when no dedicated
  // gateway event exists for the action (webhooks, overwrites, kicks, etc.).
  client.on(Events.GuildAuditLogEntryCreate, (entry, guild) => processAuditEntry(entry, guild).catch((error) => logger.error('Audit persistence failed', { error: error?.message })));

  const timer = setInterval(cleanupAntiNukeState, 60_000); timer.unref?.();
  logger.info('🛡️ Wolf Security monitor registered', { events: Object.keys(EVENT_CONFIG), auditLogStream: true });
  return true;
}
