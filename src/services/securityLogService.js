...
export async function persistAuditLogEntry(db, entry) {
  try {
    const pool = getPool(db); if (!(await ensureSecurityTables(db)) || !pool || !entry?.guild?.id) return false;
    const existing = entry.id ? await pool.query(`SELECT id FROM ${TABLE_LOGS} WHERE audit_log_id = $1 ORDER BY id DESC LIMIT 1`, [entry.id]) : { rows: [] };
    const metadata = {
      ...
      options: entry.options ? { channelId: entry.options.channelId || null, count: entry.options.count || null, id: entry.options.id || null, roleName: entry.options.roleName || null, type: entry.options.type || null, membersRemoved: entry.options.membersRemoved || null, deleteMemberDays: entry.options.deleteMemberDays || null, messageId: entry.options.messageId || null } : null,
    ...
    if (existing.rows[0]?.id) {
      await enrichSecurityLog(db, existing.rows[0].id, { executorId: executor?.id || null, executorTag: executor?.tag || executor?.username || null, auditLogId: entry.id, reason: entry.reason || null, metadata });
      return existing.rows[0].id;
    }
    return persistSecurityLog(db, { guildId: entry.guild.id, eventType: `audit.${entry.action}`, severity: 'warning', executorId: executor?.id || null, executorTag: executor?.tag || executor?.username || null, targetId: entry.target?.id || entry.targetId || null, targetType: entry.target?.constructor?.name || 'unknown', auditLogId: entry.id, reason: entry.reason || null, metadata });
...
export async function listSecurityLogs(db, guildId, limit = 100) {
  if (!(await ensureSecurityTables(db))) return []; const pool = getPool(db); if (!pool) return [];
  const result = await pool.query(`SELECT * FROM ${TABLE_LOGS} WHERE guild_id = $1 ORDER BY created_at DESC LIMIT $2`, [guildId, Math.min(Number(limit) || 100, 500)]); return result.rows;
}
export async function listSecurityIncidents(db, guildId, limit = 50) {
  if (!(await ensureSecurityTables(db))) return []; const pool = getPool(db); if (!pool) return [];
  const result = await pool.query(`SELECT * FROM ${TABLE_INCIDENTS} WHERE guild_id = $1 ORDER BY created_at DESC LIMIT $2`, [guildId, Math.min(Number(limit) || 50, 200)]); return result.rows;
}
